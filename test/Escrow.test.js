...
contract('Escrow', (accounts) => {
  ...
  beforeEach(async () => {
    ...
    TestUSDCContractInstance.transfer(EscrowContractInstance.address, ether('10000').toString(), { from: john });
  });
  describe('--- Transfer ---', async () => {
    it('should allow transfer from contract to address by owner', async () => {
      await EscrowContractInstance.transfer(bob, ether('1000').toString(), { from: alice });
      ...
      expect(bobBalance).to.be.bignumber.equal(ether('1000'));
      expect(escrowContractBalance).to.be.bignumber.equal(ether('9000'));
    });
    it('should revert if called by other than owner', async () => {
      await expectRevert(
        EscrowContractInstance.transfer(bob, ether('1000').toString(), { from: bob }),
  ...
  describe('--- Approve ---', async () => {
    it('should allow approve from contract to address by owner', async () => {
      await EscrowContractInstance.approve(bob, ether('1000'), { from: alice });
      const bobAllowance = await EscrowContractInstance.allowance(EscrowContractInstance.address, bob);
      expect(bobAllowance).to.be.bignumber.equal(ether('1000'));
    });
    it('should revert if called by other than owner', async () => {
      await expectRevert(
        EscrowContractInstance.approve(bob, ether('1000'), { from: bob }),
        'Ownable: caller is not the owner'
      );
    });
  });
